...
type Job = { id: string; filename: string; status: Status; stage: string; progress: number; createdAt: string; duration?: number; segments?: Segment[] };

const API = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080/api";
const seed: Job[] = [
  { id: "demo-001", filename: "observasi-engineering.m4a", status: "completed", stage: "done", progress: 100, createdAt: new Date(Date.now()-35*60_000).toISOString(), duration: 184, segments: [
  ...
  { id: "demo-002", filename: "daily-ops-14-aug.wav", status: "processing", stage: "diarizing", progress: 72, createdAt: new Date(Date.now()-9*60_000).toISOString(), duration: 428 },
];
...
export default function Home() {
  ...
  return <main>
    ...
    <div className="workspace"><aside><div className="sectionTitle"><span>RECENT RECORDINGS</span><b>{jobs.length}</b></div><div className="jobs">{jobs.map(j=><button key={j.id} onClick={()=>setSelected(j.id)} className={`job ${selected===j.id?"active":""}`}><span className={`status ${j.status}`}/><span className="jobcopy"><strong>{j.filename}</strong><small>{j.status==="processing"?`${j.stage} · ${j.progress}%`:j.status==="queued"?"menunggu worker":ago(j.createdAt)}</small>{j.status==="processing"&&<i style={{width:`${j.progress}%`}}/>}</span><span>›</span></button>)}</div></aside>
